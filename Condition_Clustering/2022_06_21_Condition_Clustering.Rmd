...
save(dabtram_first_pearson, dabtram_first_pearson_rand, dabtram_first_subset, dabtram_first_subset_rand, dabtram_first_pearson_filt, dabtram_first_pearson_filt_rand, dabtram_second_pearson, dabtram_second_pearson_rand, dabtram_second_subset, dabtram_second_subset_rand, dabtram_second_pearson_filt, dabtram_second_pearson_filt, file = '2022_01_14_analysis_scripts/2022_05_27_analysis/Condition_Clustering/dabtram_pearson_results.RData')
...
# Statistical analysis and plotting of pearson metrics
```{r}

## Load in data one at a time and analyze
...
pearson_ttest_cocl2_firstvssecond <- t.test(cocl2_first_pearson_filt, cocl2_second_pearson_filt, alternative = 'greater', paired = F)

plotting_df_pearson_cocl2 <- data.frame(Grouping = c(rep('first',2*length(c(cocl2_first_pearson_filt))),rep('second', 2*length(c(cocl2_second_pearson_filt)))),
                          ...
                          Treatment = c(rep('CoCl2 first',length(cocl2_first_pearson_filt)), rep('CoCl2 first rand',rep('CoCl2 second',length(cocl2_second_pearson_filt)),rep('CoCl2 second rand',length(cocl2_second_pearson_filt_rand)))))
colnames(plotting_df_pearson_cocl2) <- c('Grouping','Pearson','Treatment')

ggplot(plotting_df_pearson_cocl2, aes(x = Treatment, y = Pearson, fill = Grouping)) + geom_violin() + geom_boxplot(width = 0.1)

#Dabtram
load('2022_01_14_analysis_scripts/2022_05_27_analysis/Condition_Clustering/dabtram_pearson_results.RData')

#Cisplatin
load('2022_01_14_analysis_scripts/2022_05_27_analysis/Condition_Clustering/cis_pearson_results.RData')

```

# Look at this excluding the cells that stayed in the same drug both times???

# plotting_df_pearson <- data.frame(Grouping = c(rep('first',2*length(c(cocl2_first_pearson_filt,dabtram_first_pearson,cis_first_pearson_filt))),rep('second', 2*length(c(cocl2_second_pearson_filt,dabtram_second_pearson_filt,cis_second_pearson_filt)))),
#                           Pearson <- c(cocl2_first_pearson_filt, cocl2_first_pearson_filt_rand, dabtram_first_pearson_filt, dabtram_first_pearson_filt_rand, cis_first_pearson_filt, cis_first_pearson_filt_rand, cocl2_second_pearson_filt, cocl2_second_pearson_filt_rand, dabtram_second_pearson_filt, dabtram_second_pearson_filt_rand, cis_second_pearson_filt, cis_second_pearson_filt_rand),
#                           Treatment = c(rep('CoCl2 first',length(cocl2_first_pearson_filt)), rep('CoCl2 first rand',length(cocl2_first_pearson_filt_rand)),rep('Dab/Tram first',length(dabtram_first_pearson_filt)),rep('Dab/Tram first rand',length(dabtram_first_pearson_filt_rand)),rep('Cisplatin first',length(cis_first_pearson_filt)),rep('Cisplatin first rand',length(cis_first_pearson_filt_rand)),rep('CoCl2 second',length(cocl2_second_pearson_filt)),rep('CoCl2 second rand',length(cocl2_second_pearson_filt_rand)),rep('Dab/Tram second',length(dabtram_second_pearson_filt)),rep('Dab/Tram second rand',length(dabtram_second_pearson_filt_rand)),rep('Cisplatin second',length(cis_second_pearson_filt)),rep('Cisplatin second rand',length(cis_second_pearson_filt_rand))))
# colnames(plotting_df_pearson) <- c('Grouping','Pearson','Treatment')
...
ggplot(plotting_df_pearson, aes(x = Treatment, y = Pearson, fill = Grouping)) + geom_violin() + geom_boxplot(width = 0.1)

pearson_ttest_cisplatin <- t.test(cis_first_pearson_filt, cis_second_pearson_filt, alternative = 'less', paired = F)
pearson_ttest_cocl2 <- t.test(cocl2_first_pearson_filt, cocl2_second_pearson_filt, alternative = 'less', paired = F)
pearson_ttest_dabtram <- t.test(dabtram_first_pearson_filt, dabtram_second_pearson_filt, alternative = 'less', paired = F)