...
#Get list of relevant lineages for CoCl2:
```{r include = FALSE}

number_of_cells_threshold <- 5

#Get number of cells per lineage:
cocl2_lineages <- table(all_data$Lineage[all_data$OG_condition == "cocl2"])

#Remove still multiple and no barcode:
cocl2_lineages <- cocl2_lineages[!(row.names(cocl2_lineages) %in% c("Still multiple","No Barcode"))]

#Keep lineages with more than 5 cells:
cocl2_lineages_thresholded <- cocl2_lineages[cocl2_lineages>=number_of_cells_threshold]
...
#Remove still multiple and no barcode:
cocl2tococl2_lineages <- cocl2tococl2_lineages[!(row.names(cocl2tococl2_lineages) %in% c("Still multiple","No Barcode"))]

#Keep lineages with more than 5 cells:
cocl2tococl2_lineages_thresholded <- cocl2tococl2_lineages[cocl2tococl2_lineages>=number_of_cells_threshold]




cocl2_and_cocl2tococl2_lineages <- Reduce(intersect,list(names(cocl2_lineages_thresholded)
,names(cocl2tococl2_lineages_thresholded)
))
...
for (current_lineage in cocl2_and_cocl2tococl2_lineages){
  ...
  #Find markers higher in cocl2tococl2:
  markers_cocl2tococl2_high <- FindMarkers(data_for_current_lineage, ident.1 = 'cocl2tococl2', ident.2 = 'cocl2', only.pos = TRUE)
  
  #Find markers higher in cocl2:
  markers_cocl2_high <- FindMarkers(data_for_current_lineage, ident.1 = 'cocl2', ident.2 = 'cocl2tococl2', only.pos = TRUE)
  
  #Find go terms:
  go_cocl2tococl2_high <- enrichGO(rownames(markers_cocl2tococl2_high), OrgDb = org.Hs.eg.db, keyType = "SYMBOL")
  go_cocl2_high <- enrichGO(rownames(markers_cocl2_high), OrgDb = org.Hs.eg.db, keyType = "SYMBOL")
  
  #Make temporary dataframes, if no go terms make the temporary dataframes empty (so they have the correct number of columns):
  if (length(go_cocl2tococl2_high$qvalue) > 0){
    temp_df_cocl2tococl2_high <- data.frame(rep(current_lineage, times = length(go_cocl2tococl2_high$qvalue)), go_cocl2tococl2_high$Description, -log10(go_cocl2tococl2_high$qvalue))
  }else{
    temp_df_cocl2tococl2_high <- data.frame(matrix(ncol = 3, nrow = 0))
  }
  
  if (length(go_cocl2_high$qvalue) > 0){
    temp_df_cocl2_high <- data.frame(rep(current_lineage, times = length(go_cocl2_high$qvalue)), go_cocl2_high$Description, log10(go_cocl2_high$qvalue))
  }else{
    temp_df_cocl2_high <- data.frame(matrix(ncol = 3, nrow = 0))
  }
  
  colnames(temp_df_cocl2tococl2_high) <- c('Lineage','GO_Term','log10_qvalue')
  colnames(temp_df_cocl2_high) <- c('Lineage','GO_Term','log10_qvalue')

  
  df <- rbind(df,temp_df_cocl2tococl2_high,temp_df_cocl2_high)
  
}

#Save dataframe:
write.csv(df, "2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/MultipleLineages_GO_CoCl2.csv")
...
df <- read.csv("2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/MultipleLineages_GO_CoCl2.csv", row.names = 1)

#Add overall GO-terms:

overall_df_up <- read.csv("2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/GO_cocl2_Vs_cocl2Tococl2.csv")

overall_df_down <- read.csv("2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/GO_cocl2Tococl2_Vs_cocl2.csv")
...
#for(lineage in colnames(df_casted)){
    ...
    #colnames(df_casted)[colnames(df_casted) == lineage] <- paste(lineage,", ",cocl2_lineages[lineage],"->",cocl2tococl2_lineages[lineage], sep = "")
  #}
#}
...
for(lineage in colnames(df_casted)){
  if(lineage != "All_Cells"){
    colnames(df_casted)[colnames(df_casted) == lineage] <- paste(round(cocl2_lineages[lineage]*100.0/sum(cocl2_lineages),2),"->",round(cocl2tococl2_lineages[lineage]*100.0/sum(cocl2tococl2_lineages),2), sep = "")
  }
}
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/CoCl2_Lineage_Heatmap_Cellular_Respiration.pdf', family = "ArialMT")
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/CoCl2_Lineage_Heatmap_cell_cell_communication.pdf', family = "ArialMT")
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/CoCl2_Lineage_Heatmap_extracellular_binding.pdf', family = "ArialMT")
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/CoCl2_Lineage_Heatmap_transcription_translation.pdf', family = "ArialMT")
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/CoCl2_Lineage_Heatmap_MHC.pdf', family = "ArialMT")
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/CoCl2_Lineage_Heatmap_ubiquitin.pdf', family = "ArialMT")
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/CoCl2_Lineage_Heatmap_enzyme.pdf', family = "ArialMT")
