...
#Get list of relevant lineages for cis:
```{r include = FALSE}
...
cis_lineages <- cis_lineages[!(row.names(cis_lineages) %in% c("Still multiple","No Barcode"))]

#Keep lineages with more than 5 cells:
cis_lineages_thresholded <- cis_lineages[cis_lineages>=number_of_cells_threshold]
...
#Remove still multiple and no barcode:
cistocis_lineages <- cistocis_lineages[!(row.names(cistocis_lineages) %in% c("Still multiple","No Barcode"))]

#Keep lineages with more than 5 cells:
cistocis_lineages_thresholded <- cistocis_lineages[cistocis_lineages>=number_of_cells_threshold]




cis_and_cistocis_lineages <- Reduce(intersect,list(names(cis_lineages_thresholded)
,names(cistocis_lineages_thresholded)
))
...
for (current_lineage in cis_and_cistocis_lineages){
  ...
  #Find markers higher in cistocis:
  markers_cistocis_high <- FindMarkers(data_for_current_lineage, ident.1 = 'cistocis', ident.2 = 'cis', only.pos = TRUE)
  
  #Find markers higher in cis:
  markers_cis_high <- FindMarkers(data_for_current_lineage, ident.1 = 'cis', ident.2 = 'cistocis', only.pos = TRUE)
  
  #Find go terms:
  go_cistocis_high <- enrichGO(rownames(markers_cistocis_high), OrgDb = org.Hs.eg.db, keyType = "SYMBOL")
  go_cis_high <- enrichGO(rownames(markers_cis_high), OrgDb = org.Hs.eg.db, keyType = "SYMBOL")
  
  #Make temporary dataframes, if no go terms make the temporary dataframes empty (so they have the correct number of columns):
  if (length(go_cistocis_high$qvalue) > 0){
    temp_df_cistocis_high <- data.frame(rep(current_lineage, times = length(go_cistocis_high$qvalue)), go_cistocis_high$Description, -log10(go_cistocis_high$qvalue))
  }else{
    temp_df_cistocis_high <- data.frame(matrix(ncol = 3, nrow = 0))
  }
  
  if (length(go_cis_high$qvalue) > 0){
    temp_df_cis_high <- data.frame(rep(current_lineage, times = length(go_cis_high$qvalue)), go_cis_high$Description, log10(go_cis_high$qvalue))
  }else{
    temp_df_cis_high <- data.frame(matrix(ncol = 3, nrow = 0))
  }
  
  colnames(temp_df_cistocis_high) <- c('Lineage','GO_Term','log10_qvalue')
  colnames(temp_df_cis_high) <- c('Lineage','GO_Term','log10_qvalue')

  
  df <- rbind(df,temp_df_cistocis_high,temp_df_cis_high)
  
}

#Save dataframe:
write.csv(df, "2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/MultipleLineages_GO_cis.csv")
...
df <- read.csv("2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/MultipleLineages_GO_cis.csv", row.names = 1)

#Add overall GO-terms:

overall_df_up <- read.csv("2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/GO_cis_Vs_cisTocis.csv")

overall_df_down <- read.csv("2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/GO_cisTocis_Vs_cis.csv")
...
#for(lineage in colnames(df_casted)){
    ...
    #colnames(df_casted)[colnames(df_casted) == lineage] <- paste(lineage,", ",cis_lineages[lineage],"->",cistocis_lineages[lineage], sep = "")
  #}
#}
...
for(lineage in colnames(df_casted)){
  if(lineage != "All_Cells"){
    colnames(df_casted)[colnames(df_casted) == lineage] <- paste(round(cis_lineages[lineage]*100.0/sum(cis_lineages),2),"->",round(cistocis_lineages[lineage]*100.0/sum(cistocis_lineages),2), sep = "")
  }
}
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/cis_Lineage_Heatmap_Cellular_Respiration.pdf', family = "ArialMT")
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/cis_Lineage_Heatmap_cell_cell_communication.pdf', family = "ArialMT")
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/cis_Lineage_Heatmap_extracellular_binding.pdf', family = "ArialMT")
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/cis_Lineage_Heatmap_transcription_translation.pdf', family = "ArialMT")
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/cis_Lineage_Heatmap_MHC.pdf', family = "ArialMT")
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/cis_Lineage_Heatmap_ubiquitin.pdf', family = "ArialMT")
...
pdf('2022_01_14_analysis_scripts/2022_05_27_analysis/pathway_analysis/Is_Resistance_Additive/cis_Lineage_Heatmap_enzyme.pdf', family = "ArialMT")
