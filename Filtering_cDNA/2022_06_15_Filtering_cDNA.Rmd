...
DabTram_df <- merge(dabtram_lineages, gDNA_collapsed$DabTram, by = 'Lineage' )
...
# this object is unfiltered for both datasets
gDNA_cDNA_collapsed <- list(Cis = Cis_df, CistoCis = CistoCis_df, CistoCoCl2 = CistoCoCl2_df, CistoDabTram = CistoDabTram_df, CoCl2 = CoCl2_df, CoCl2toCis = CoCl2toCis_df, CoCl2toCoCl2 = CoCl2toCoCl2_df, 
                       CoCl2toDabTram = CoCl2toDabTram_df, DabTram = DabTram_df, DabTramtoCis = DabTramtoCis_df, DabTramtoCoCl2 = DabTramtoCoCl2_df, DabTramtoDabTram = DabTramtoDabTram_df)
...
# Export CSV with # reads for 1, 2 cells per condition, want to compare this to the RPM cutoffs previously decided upon for 50 cell
```{r include = FALSE}
...
for (i in names(filtered_lins_list)){
  ...
  gDNA_cDNA_unfilt <- list(cDNA = gDNA_cDNA_collapsed[[i]]$Lineage, gDNA = gDNA_cDNA_collapsed[[i]]$Lineage[gDNA_cDNA_collapsed[[i]]$Reads > 0])
  ...
  gDNA_unfilt_cDNA_filt <- list(cDNA = gDNA_cDNA_collapsed[[i]]$Lineage[gDNA_cDNA_collapsed[[i]]$Num_Cells > 1], gDNA = gDNA_cDNA_collapsed[[i]]$Lineage[gDNA_cDNA_collapsed[[i]]$Reads > 0])
...
#cDNA_collapsed num_cells greater than 50 for each condition
...
# Find induced resistant lineages in merged list
```{r}

dabtram_cocl2 <- list(dabtram = unique(c(combined_lins_list$DabTram, combined_lins_list$DabTramtoCis, combined_lins_list$DabTramtoCoCl2, combined_lins_list$DabTramtoDabTram)), 
                           cocl2 = unique(c(combined_lins_list$CoCl2, combined_lins_list$CoCl2toCis, combined_lins_list$CoCl2toCoCl2, combined_lins_list$CoCl2toDabTram)),
                           dabtramtococl2 = combined_lins_list$DabTramtoCoCl2,
                           cocl2todabtram = combined_lins_list$CoCl2toDabTram)
dabtram_cocl2_venn <- venn(dabtram_cocl2, intersections = T)

dabtram_cis <- list(dabtram = unique(c(combined_lins_list$DabTram, combined_lins_list$DabTramtoCis, combined_lins_list$DabTramtoCoCl2, combined_lins_list$DabTramtoDabTram)), 
                         cis = unique(c(combined_lins_list$Cis, combined_lins_list$CistoCis, combined_lins_list$CistoCoCl2, combined_lins_list$CistoDabTram)),
                         dabtramtocis = combined_lins_list$DabTramtoCis,
                         cistodabtram = combined_lins_list$CistoDabTram)
dabtram_cis_venn <- venn(dabtram_cis, intersections = T)

cocl2_cis <- list(cocl2 = unique(c(combined_lins_list$CoCl2, combined_lins_list$CoCl2toCis, combined_lins_list$CoCl2toCoCl2, combined_lins_list$CoCl2toDabTram)),
                       cis = (c(combined_lins_list$Cis, combined_lins_list$CistoCis, combined_lins_list$CistoCoCl2, combined_lins_list$CistoDabTram)),
                       cocl2tocis = combined_lins_list$CoCl2toCis,
                       cistococl2 = combined_lins_list$CistoCoCl2)
cocl2_cis_venn <- venn(cocl2_cis, intersections = T)
...
save(combined_lins_list, induced_resistant_lins, overlapping_lins, fivecell_cDNA, file = '2022_01_14_analysis_scripts/2022_05_27_analysis/Filtering_cDNA/resistant_lineage_lists.RData')
```