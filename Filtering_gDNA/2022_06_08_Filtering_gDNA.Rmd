...
# identify the spike-in lineages within each sample -- these were found by searching for the known barcode sequence in the gDNA anno object and finding the lineage name for each
...
for (i in names(gDNA_collapsed)){
  ...
  #run regression and make cutoffs based upon
...
#how do these look vs first sample
```{r}
...
# actually making the filtering
```{r}

# for heatmaps we want these to have the same dimensions but for gene expression integration the NAs are going to get annoying

# for now will just work on firstcondition50
